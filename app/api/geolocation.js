...
function validarDados(dados) {
    var retorno = sucesso;
    if (dados.pontoOrigem === undefined) {
        ...
        falha.erro = "O 'destinos' deve conter um array de objetos com propriedades 'lat' e 'lon' (latitude e longitude respectivamente) e ambos devem ser maiores que zero.";
        retorno = falha;
    } else if (dados.destinos.length > 0) {
        var contador = 0;
        dados.destinos.forEach(element => {
            if (element.lat === undefined || element.lon === undefined || element.lat === 0 || element.lon === 0) {
                falha.erro = "O destino no index " + contador + " não pode conter um de seus valores nulo, indefinido ou zero.";
                retorno = falha;
                return false;
            }
            contador++;
